...
**Implementation evidence (2026-08-28):** The command palette now says “Find a page or action…” and the shell labels its trigger as commands and navigation. `apps/web/src/app/navigation.ts` now exposes a combined `routeManifest` and `routeMetaForPath()` lookup so route metadata can drive shells, breadcrumbs, contextual actions, and document titles from one manifest; `AppRouter` applies metadata titles on navigation. `apps/web/test/navigation.test.ts` verifies unique route entries and query-bearing path resolution. Domain search remains a separate future capability.
