...
**Additional evidence (2026-08-29):** Migration `0040_platform_feature_flag_overrides` and the shared `FeatureFlagOverrideResponse` contract establish durable global/plan/tenant/pilot scope, effective-window, actor, reason, and previous-value fields. Drizzle/in-memory adapters and authenticated Platform `GET/POST /platform/feature-flag-overrides` endpoints now validate registry keys, scope/value combinations, and temporal windows, with audit events. `/platform/feature-flags` now provides the dedicated registry-backed create/history screen; runtime evaluation semantics remain open.
