...
Current verified slices include membership hold/resume/renewal, durable account export/plan-change/cancellation/deletion requests with Platform metadata and review decisions, shared recoverable query failures across Ops, Insights, Bookings, Members, Attendance, Memberships, Schedule, Services, Equipment, and Platform overview/directory, stable-only SaaS pricing claims backed by the capability registry, booking reschedule and waitlist promotion, Coach roster signals, tenant-scoped member tags with assignment/filter CRUD, reusable member-segment definitions, tenant/user-scoped saved member views, permission-safe bulk member status changes with per-record audit, tenant/branch-scoped global tasks with assignee, lifecycle, and comment persistence, branch-aware CRM workload/overdue follow-up metrics, and the converted-lead trial-booking workflow. Each slice has explicit roadmap evidence and local verification; hosted browser coverage remains separate. Queue schemas and API handoffs remain non-fulfillment until a worker consumer updates durable state.
