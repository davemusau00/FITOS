...
The Sites contract, persisted-page editing, Inventory Receive Lot foundations, persisted notification inbox, scoped feature-flag evaluation, Platform support notes, account-recovery cases, scoped system notices, guarded implementation inquiry conversion, Ops six-hour signal board, booking reschedule/promotion, staff waitlist join/leave/reorder and automatic promotion, entitlement override capture, Coach roster signals, member-tag persistence/filtering, reusable member-segment definitions, per-user saved member views, permission-safe bulk member status actions, the persisted global task queue with handoff comments, CRM workload/overdue follow-up aggregation, and the converted-lead trial-booking workflow are implemented and locally PostgreSQL-verified in the roadmap evidence ledger. They still require hosted/browser acceptance where noted; the next active product blockers are Playwright CI, notification producer coverage, all-location semantics, specialist workflow completion, and broader member/CRM workflows.
...
Current verified slices include membership hold/resume/renewal, durable account export/plan-change/cancellation/deletion requests with Platform metadata and review decisions, shared recoverable query failures across Ops, Insights, Bookings, Members, Attendance, Memberships, Schedule, Services, Equipment, and Platform overview/directory, stable-only SaaS pricing claims backed by the capability registry, booking reschedule, staff waitlist join/leave/reorder and automatic promotion with equipment-constrained capacity checks, entitlement override capture, server-derived Reception attendance state with occurrence-linked session check-in, Coach roster signals, tenant-scoped member tags with assignment/filter CRUD, reusable member-segment definitions, tenant/user-scoped saved member views, permission-safe bulk member status changes with per-record audit, tenant/branch-scoped global tasks with assignee, lifecycle, and comment persistence, branch-aware CRM workload/overdue follow-up metrics, and the converted-lead trial-booking workflow. Each slice has explicit roadmap evidence and local verification; hosted browser coverage remains separate. Queue schemas and API handoffs remain non-fulfillment until a worker consumer updates durable state.
