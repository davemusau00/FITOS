...
Membership hold, resume, and renewal are examples of the required bar: each is persisted, permission-gated, audited, covered by integration tests, and surfaced with recoverable UI feedback. Account export, plan-change, cancellation, and deletion requests now meet the same local persistence/audit boundary, with explicit confirmation for deletion and no automatic destructive execution. The notification inbox now meets the local persistence, ownership, read-state, deep-link, and recoverable UI bar for those lifecycle events. Platform support notes and account-recovery cases now persist tenant scope, actor/evidence, audit history, and recoverable UI state, with tenant-scoped staff-session revocation for verified recovery subjects. Scoped system notices now meet the scheduling, expiry, audience filtering, per-user acknowledgement, and recoverable UI bar. Booking reschedule, staff waitlist join/leave/reorder and automatic promotion, entitlement-override capture, Coach roster signals, member-tag assignment/filtering, reusable member-segment definitions, per-user saved member views, persisted global task lifecycle, persisted task comments, branch-aware CRM workload/overdue follow-up metrics, and converted-lead trial booking now meet the local contract/persistence/audit boundary described in the roadmap, while the status-only bulk action, server-side Today aggregation, resource-aware booking capacity, member-safe waitlist messaging, booking history/rescheduling from CRM, and deep-linked task surfaces remain partial pending broader workflows. Shared query failures likewise expose a keyboard-operable retry action on the verified tenant and Platform surfaces. Evidence is recorded in the roadmap matrix; payment collection is intentionally excluded.
