...
import type { FitosRepository, InviteStaffInput, StaffAccessInput, TenantScope } from "../../ports/fitos-repository.js";
...
export class CoreService {
  ...
  async createBranch(actor: RequestActor, requestId: string, input: CreateBranchRequest): Promise<BranchResponse> {
    try {
      const branch = await this.repository.createBranch(scopeOf(actor), input);
      await this.audit(actor, requestId, "branch.created", "branch", branch.id, branch.id, { name: branch.name, slug: branch.slug });
      await this.publish(eventOf(actor, "branch.created", { branchId: branch.id }));
      return branch;
    } catch (error) {
      if (error instanceof Error && error.message.includes("slug")) {
        throw new DomainError("VALIDATION_FAILED", "A branch with that slug already exists.", 409, { slug: ["Must be unique within this organization."] });
      }
      throw error;
    }
  }

  async updateBranch(actor: RequestActor, requestId: string, branchId: string, input: UpdateBranchRequest): Promise<BranchResponse> {
    ...
    try {
      ...
      await this.audit(actor, requestId, updated.isActive ? "branch.updated" : "branch.deactivated", "branch", updated.id, updated.id, {
        changed: Object.keys(input),
        beforeActive: before.isActive,
        afterActive: updated.isActive
      });
      await this.publish(eventOf(actor, updated.isActive ? "branch.updated" : "branch.deactivated", { branchId: updated.id }));
      return updated;
    } catch (error) {
      if (error instanceof Error && error.message.includes("slug")) {
        throw new DomainError("VALIDATION_FAILED", "A branch with that slug already exists.", 409, { slug: ["Must be unique within this organization."] });
  ...
  async listMembers(actor: RequestActor, filters: MemberListFilters): Promise<CursorPage<MemberListItem>> {
  ...
  async createMember(actor: RequestActor, requestId: string, input: CreateMemberRequest): Promise<MemberResponse> {
    if (!actor.branchIds.includes(input.homeBranchId)) {
      throw new DomainError("BRANCH_ACCESS_DENIED", "You cannot create a member for this branch.", 404);
    }
    ...
    try {
      ...
      await this.audit(actor, requestId, "member.created", "member", member.id, member.homeBranchId, {
  ...
  async updateMember(actor: RequestActor, requestId: string, memberId: string, input: UpdateMemberRequest): Promise<MemberResponse> {
    const before = await this.getMember(actor, memberId);
    if (input.homeBranchId && !actor.branchIds.includes(input.homeBranchId)) {
      throw new DomainError("BRANCH_ACCESS_DENIED", "You cannot move a member to this branch.", 404);
    }
    const phone = input.contact && Object.prototype.hasOwnProperty.call(input.contact, "phone") ? this.normalizePhoneInput(input.contact.phone) : undefined;
    ...
    await this.audit(actor, requestId, input.status === "archived" ? "member.deactivated" : "member.updated", "member", updated.id, updated.homeBranchId, {
      changed: Object.keys(input),
      previousStatus: before.status,
      status: updated.status
    });
    await this.publish(eventOf(actor, input.status === "archived" ? "member.deactivated" : "member.updated", { memberId: updated.id }));
    return updated;
  }
  ...
  async inviteStaff(actor: RequestActor, requestId: string, input: InviteStaffInput): Promise<StaffUserResponse> {
    ...
    try {
      ...
      await this.audit(actor, requestId, "user.invited", "user", staff.user.id, null, { role: role.name, branches: input.branchIds });
      await this.publish(eventOf(actor, "user.invited", { userId: staff.user.id }));
      return staff;
    } catch (error) {
      if (error instanceof Error && error.message.includes("already")) {
        throw new DomainError("VALIDATION_FAILED", "This person already has staff access.", 409, { email: ["Already invited or active."] });
      }
      throw error;
    }
  }

  async updateStaffAccess(actor: RequestActor, requestId: string, userId: string, input: StaffAccessInput): Promise<StaffUserResponse> {
    ...
    this.assertBranchesAccessible(actor, input.branchIds);
    if (current.role.key === "owner" && newRole.key !== "owner" && (await this.repository.countActiveOwners(scopeOf(actor))) <= 1) {
      throw new DomainError("FINAL_ACTIVE_OWNER_REQUIRED", "At least one active owner must remain.", 409);
    }
    const updated = await this.repository.updateStaffAccess(scopeOf(actor), userId, { roleId: input.roleId, branchIds: input.branchIds });
    if (!updated) throw new DomainError("RESOURCE_NOT_FOUND", "Staff user not found.", 404);
    await this.audit(actor, requestId, "user.access_changed", "user", userId, null, { fromRole: current.role.name, toRole: updated.role.name, branches: input.branchIds });
    await this.publish(eventOf(actor, "user.access_changed", { userId }));
    return updated;
  }

  async deactivateStaff(actor: RequestActor, requestId: string, userId: string): Promise<StaffUserResponse> {
    const current = await this.repository.findStaffByUserId(scopeOf(actor), userId);
    if (!current) throw new DomainError("RESOURCE_NOT_FOUND", "Staff user not found.", 404);
    if (current.role.key === "owner" && (await this.repository.countActiveOwners(scopeOf(actor))) <= 1) {
      throw new DomainError("FINAL_ACTIVE_OWNER_REQUIRED", "At least one active owner must remain.", 409);
    }
    const updated = await this.repository.deactivateStaff(scopeOf(actor), userId);
    if (!updated) throw new DomainError("RESOURCE_NOT_FOUND", "Staff user not found.", 404);
    await this.audit(actor, requestId, "user.deactivated", "user", userId, null, { role: current.role.name });
  ...
  private async requireAssignableRole(actor: RequestActor, roleId: string): Promise<RoleResponse> {
    ...
    if (!role) throw new DomainError("VALIDATION_FAILED", "Role is unavailable.", 400, { roleId: ["Unknown role."] });
    if (!role.permissions.every((permission) => actor.permissions.includes(permission))) {
      throw new DomainError("FORBIDDEN", "You cannot grant a permission that you do not hold.", 403);
    }
    return role;
  }
  ...
  private normalizePhoneInput(phone: string | null | undefined): string | null {
    ...
    if (!normalized) throw new DomainError("VALIDATION_FAILED", "Phone number is invalid.", 400, { "contact.phone": ["Enter an E.164 or Kenyan mobile number."] });
    return normalized;
  }
  ...
  private async audit(
    ...
    await this.repository.recordAudit({ tenantId: actor.tenantId, branchId, actorUserId: actor.userId, action, resourceType, resourceId, afterSummary, requestId });
  }
