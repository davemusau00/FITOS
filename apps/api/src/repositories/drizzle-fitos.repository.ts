...
export class DrizzleFitosRepository implements FitosRepository {
  ...
  private deletionResponse(row: typeof accountDeletionRequests.$inferSelect) {
    return {
      id: row.id,
      tenantId: row.tenantId,
      requestedByUserId: row.requestedByUserId,
      status: row.status as import("@fitos/contracts").AccountDeletionStatus,
  ...
  async createAccountDeletionRequest(
    ...
    const [row] = await this.db
      ...
      .values({ tenantId: scope.tenantId, requestedByUserId, confirmation, reason: reason ?? null })
      .returning();
    if (!row) throw new Error("Unable to create account deletion request.");
    return this.deletionResponse(row);
  }
  async listAccountDeletionRequests(scope: TenantScope) {
    const rows = await this.db
      .select()
      .from(accountDeletionRequests)
      .where(eq(accountDeletionRequests.tenantId, scope.tenantId))
  ...
  async getTodayOverview(
    ...
    const result = await this.db.execute<any>(
      sql`SELECT to_char(now() AT TIME ZONE ${timezone}, 'YYYY-MM-DD') AS date, (SELECT count(*) FROM members WHERE tenant_id = ${scope.tenantId} AND home_branch_id = ${branchId} AND status = 'active')::int AS active_members, (SELECT count(*) FROM members WHERE tenant_id = ${scope.tenantId} AND home_branch_id = ${branchId} AND (joined_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date)::int AS joined_today, (SELECT count(*) FROM bookings WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND (created_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date)::int AS bookings_today, (SELECT count(*) FROM bookings WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND (created_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date AND status = 'confirmed')::int AS confirmed_bookings, (SELECT count(*) FROM bookings WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND (created_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date AND status = 'cancelled')::int AS cancelled_bookings, (SELECT count(*) FROM attendance_records WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND (checked_in_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date)::int AS checked_in, (SELECT count(*) FROM attendance_records WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND (checked_in_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date AND status = 'no_show')::int AS no_shows, (SELECT count(*) FROM bookings b JOIN schedule_occurrences so ON so.id = b.occurrence_id WHERE b.tenant_id = ${scope.tenantId} AND b.branch_id = ${branchId} AND (so.starts_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date AND b.status = 'confirmed')::int AS expected_arrivals, (SELECT count(*) FROM schedule_occurrences WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND (starts_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date)::int AS sessions_today, (SELECT json_build_object('id', so.id, 'name', s.name, 'startsAt', so.starts_at) FROM schedule_occurrences so JOIN services s ON s.id = so.service_id WHERE so.tenant_id = ${scope.tenantId} AND so.branch_id = ${branchId} AND so.status = 'scheduled' AND so.starts_at >= now() ORDER BY so.starts_at LIMIT 1) AS next_session, (SELECT count(*) FROM leads WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND (created_at AT TIME ZONE ${timezone})::date = (now() AT TIME ZONE ${timezone})::date)::int AS leads_today, (SELECT count(*) FROM leads WHERE tenant_id = ${scope.tenantId} AND branch_id = ${branchId} AND next_follow_up_at IS NOT NULL AND next_follow_up_at <= now() AND stage NOT IN ('joined', 'lost'))::int AS follow_ups_due`
    );
    ...
    return {
      ...
      attendance: {
        checkedInToday: Number(row.checked_in ?? 0),
        expectedToday: Number(row.expected_arrivals ?? 0),
      ...
      schedule: {
        ...
        nextSession: row.next_session
          ? {
              id: String(row.next_session.id),
              name: String(row.next_session.name),
              startsAt: new Date(row.next_session.startsAt).toISOString()
            }
          : null
      },
      leads: {
        newToday: Number(row.leads_today ?? 0),
