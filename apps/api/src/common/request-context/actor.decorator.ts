...
export const Actor = createParamDecorator((_data: unknown, context: ExecutionContext): RequestActor => {
  const request = context.switchToHttp().getRequest<FitosRequest>();
  if (!request.actor) throw new Error("Request actor is unavailable outside an authenticated route.");
