...
export async function processOperationsJob(job: WorkerJob): Promise<void> {
  switch (job.type) {
    ...
    case "automations.execute":
      process.stdout.write(
        JSON.stringify({
          event: "automation.delivery.required",
          eventId: job.eventId,
          tenantId: job.tenantId,
...
async function dispatchNotification(job: Extract<WorkerJob, { type: "notifications.send" }>): Promise<void> {
  const { channel, data } = job.payload;
  const recipient = String(data.recipient ?? data.email ?? data.phone ?? "");
  if (!recipient) {
    process.stdout.write(JSON.stringify({ event: "notification.queued", eventId: job.eventId, tenantId: job.tenantId, channel }) + "\n");
    return;
  }
  if (channel === "email") {
    const key = process.env.RESEND_API_KEY;
    if (!key) throw new Error("RESEND_API_KEY is not configured.");
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: process.env.RESEND_FROM_EMAIL ?? "FITOS <noreply@fitos.app>", to: [recipient], subject: String(data.subject ?? job.payload.template), html: String(data.body ?? "") }) });
