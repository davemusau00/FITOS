...
import { branches, permissions, rolePermissions, roles, tenantUsers, tenants, userBranchAccess, users } from "./schema.js";

const [tenantName, tenantSlug, ownerEmail, ownerName, branchName = "Main", branchSlug = "main"] = process.argv.slice(2);
...
const roleNames: Record<RoleKey, string> = { owner: "Owner", manager: "Manager", reception: "Reception", trainer: "Trainer", finance: "Finance" };
const database = createDatabase();
try {
  const existing = await database.db.query.tenants.findFirst({ where: eq(tenants.slug, tenantSlug) });
  ...
  await database.db.transaction(async (tx) => {
    for (const key of PERMISSION_KEYS) {
      await tx.insert(permissions).values({ key, description: key.replace(":", " ") }).onConflictDoNothing({ target: permissions.key });
    }
    ...
    const [branch] = await tx.insert(branches).values({ tenantId: tenant.id, name: branchName, slug: branchSlug }).returning();
    ...
    for (const roleKey of Object.keys(roleNames) as RoleKey[]) {
      const [role] = await tx.insert(roles).values({ tenantId: tenant.id, name: roleNames[roleKey], systemKey: roleKey, isSystem: true }).returning();
      if (!role) throw new Error(`Role creation failed for ${roleKey}.`);
      roleByKey.set(roleKey, role.id);
      await tx.insert(rolePermissions).values(DEFAULT_ROLE_PERMISSIONS[roleKey].map((permissionKey) => ({ roleId: role.id, permissionKey })));
    }
    const [user] = await tx.insert(users).values({ email: ownerEmail.toLowerCase(), displayName: ownerName, passwordHash }).returning();
    const ownerRoleId = roleByKey.get("owner");
    if (!user || !ownerRoleId) throw new Error("Owner creation failed.");
    const [tenantUser] = await tx.insert(tenantUsers).values({ tenantId: tenant.id, userId: user.id, roleId: ownerRoleId }).returning();
